...
//Functions were implemented based on the BACK-PROP-LEARNING neural network algorithm referenced from the textbook,
//"Artificial Intelligence A Modern Approach Third Edition by Stuart J. Russell and Peter Norvig", located on pages.734.
//Author of implementations on function computeOutputDelta() and computeHiddenDelta() : Jacky Z. Chen : Last edited Dec 11th 2017
//Course: Introduction to Artificial Intelligence; CSE 175 UC MERCED FALL 2017
...
public class Layer {
	...
	public void computeOutputDelta() {
		...
		//Create two new vectors; one for total change over time & the other for target act differences
		Vector netChange = new Vector(net.derivative(min, max));
		Vector TAdiff = new Vector(targ.difference(act));

		//Loop through all units in layer
		for(int i = 0; i < n; i++) {
			/*Calculate the unit delta vector values for output layer 
			based on the product of target act differences and the total change over time*/
			delta.set(i, (TAdiff.get(i)) * (netChange.get(i)));
		}
	...
	public void computeHiddenDelta() {
		...
		//Create two new vectors; one for differences & other for the total change over time
		Vector diff = new Vector(n);
		Vector netChange = new Vector(net.derivative(min, max));

		//Loop through downstream layer projections
		for(Projection projections: outputs) {
			//Create a vector which computes the transpose of weight and multiply by each output delta based on the projections
			Vector downstream_delta_layer_values = projections.W.transpose().product(projections.output.delta);
			//differences is the sum of the computed downstream layer values
			diff = diff.sum(downstream_delta_layer_values);
		}

		//Loop through all units in layer
		for(int i = 0; i < n; i++) {
			//Calculate the unit delta values for hidden layer 
			//based on the product of differences and net change over entire range
			delta.set(i, (diff.get(i)) * (netChange.get(i)));
		}
