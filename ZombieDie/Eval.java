...
//
// Author of implementation on functions: static double value_roll_hand(State s, int depth) {} 
//and static public double heuristic(State s) {} : Jacky Z. Chen -- Last edited : Mon Dec 4th 2017
...
public class Eval {
	...
	static double value_roll_hand(State s, int depth) {
		...
		//Loop through three dices of all possible combinations and to ensure those values are valid
		//if so compute the expected utility value for each of these scenarios
		for (DieFace dFace1 : DieFace.values()) {
			if (dFace1 != (DieFace.invalid)) {
				for (DieFace dFace2 : DieFace.values()) {
					if (dFace2 != (DieFace.invalid)) {
						for (DieFace dFace3 : DieFace.values()) {
							if (dFace3 != (DieFace.invalid)) {
								//Calling function rollProb which returns the probability of rolling a specific dice in hand
								//as well as assuming there exists three dices, specifically dFace1, dFace2, and dFace3
								//and storing them in the variable "probability" with data type of double since it's probability
								double probability = s.rollProb(dFace1, dFace2, dFace3);
								//State consists of rolling three dices (start rolling)
								s.roll(dFace1, dFace2, dFace3);
								//value_rolled_hand has been called upon computing the expected value
								double eVals = Eval.value_rolled_hand(s, depth);
								//Computes expected utility value:
								//equation for expected value is essentially value times probability then sum of all (probability & statistics)
								val = val + ((eVals) * (probability));
							}
						}
					}
				}
	...
	static public double heuristic(State s) {
		...
		//initialization; brains/blasts collected and postiive/negative payoff
		int brainsCount = s.brains_collected;
		int blastsCount = s.blasts_collected;
		double negativeReinforcement = (-1*State.win_payoff);
		double positiveReinforcement = State.win_payoff;

		//a value that determines the differences between user and computer
		double userdiffcomp = ((s.comp_brains_eaten) - (s.user_brains_eaten));
		//heuristic value that determines the expected winning percentage among the differences 
		double winCount = 13;
		value = ((userdiffcomp/winCount)*100);

		//if it is computer's turn
		if (s.current_player == Turn.computer) {
			//if computer chooses to continue rolling
			if (s.current_choice == Choice.roll) {
				//if 2 or more blasts are collected, bad!!! WATCH OUT.
				if (blastsCount >= 2) {
					return negativeReinforcement;
				}
				//else if the blasts are one and two inclusive AND brains collected are greater than one, return goodness!
				else if (blastsCount >= 1 && blastsCount <=2 && brainsCount >= 1) {
					return positiveReinforcement;
				}
				//else just return heuristic value
				else {
					return value;
				}
			}
			//else if computer chooses to stop
			else if (s.current_choice == Choice.stop) {
				//if blasts collected are less than or equal to 2 and brains collected exceed or equal to one, return goodness!
				if (blastsCount >= 1 && brainsCount >= 1) {
					return positiveReinforcement;
				}
				//else just return heuristic value
				else {
